...
public:
  ...
  void checkTimerAlert() {
    // Si el tiempo se agotó y no hemos enviado notificación aún
    if (getRemainingMs() == 0 && isRunning && !isPaused && !notificationSent) {
      NotifyManager::instance().send(
        "⏰ Timer Completado",
        "¡El tiempo ha terminado!\n<b>Click en el timer para reiniciar.</b>",
        NOTIFY_URGENCY_CRITICAL
      );
      notificationSent = true;
    }

    // Resetear el flag si el timer se reinicia o se pausa
